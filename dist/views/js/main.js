...
//optimized in class. got rid of determineDx.
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    //declared pizzaLen so the "for-loop" doesn't have to calculate it everytime
    var pizzaLen = randomPizzas.length;

    //cached randomPizzas, made loop simplified
...
//moved outside of loop for optimization
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');
  //cached
  var pizzaCach = items.length;
  //declare array
  var modArray = [];
  //declare i outside of loop
  var i;
  //push number in an array for modulo
  for (i = 0; i < 5; i++) {
    modArray.push(Math.sin((dScroll / 1250) + (i)));
  }

  //var items = document.getElementsByClassName('mover');
  //used numbers from array x modulo to move background pizzas via transform
  for (i = 0; i < pizzaCach; i++) {
    var phase = modArray[i % 5];
    //var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5));
    //items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
    //var moving = items[i].basicLeft + 100 * phase + 'px';
...
window.addEventListener('scroll', updatePositions); //requestAnimationFrameScrolling);
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //using innerHeight to calculate number of background pizzas painted based on screen size
  ...
  for (var i = 0; i < pizzaDelivery; i++) {
    ...
    //coordinating with with translateX
    elem.style.left = (i % cols) * s + 'px';
    //elem.basicLeft = (i % cols) * s;
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    //changed querySelectorAll to getElementById
